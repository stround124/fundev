    ...
    match button.id:
        ...
        case _:
            ...
            match button.id:
                ...
                case "x":
                    lblQuestion = toga.Label(x_zad(curr_menu.app), margin=20)
                case _:
...
def x_zad(app):
    ...
    random.random(1, j-1)
    if rand == 1:
        return f"Пресметни {num1} + {app.resTrue} = x"
        app.resTrue = num1 + num2
    elif rand == 2:
        app.resTrue = num1 - num2
        return f"Пресметни {num1} - {app.resTrue} = x"
        ...
        app.resTrue = num1 // num2 if num2 != 0 else 0
        return f"Пресметни {num1} / {app.resTrue} = x"
...
class FunMathGUI(toga.App):
    ...
    def main_menu(self):
        ...
        btnDrobi = toga.Button("Дроби", id="drobi", on_press=onButtonClick, margin_top=20)
        btnX = toga.Button("Задачи с X", id="x_zad", on_press=onButtonClick)
    ...
    def draw_logo(self):
        ...
        self.text_width, text_height = self.canvas_logo.measure_text("FunMath GUI", font)
